...
const DEFAULT_LOGO_IMAGE = '/shared/%D0%90%D0%B2%D0%B0%D1%82%D0%B0%D1%80%D0%BA%D0%B0.png';
const DEFAULT_DISPLAY_GROUPS = [
  { id: 8821, key: 'standart-b', title: 'Общий зал', order: 1 },
  { id: 8818, key: 'vip', title: 'VIP', order: 2 },
  { key: 'tv', title: 'TV', source: 'timer', apiUrl: 'http://localhost:8080/api/display-zone', order: 3 }
];
...
function normalizeDisplayGroup(group, index) {
  ...
  return {
    ...
    ids: [...new Set(ids)],
    source: group.source === 'timer' ? 'timer' : 'smartshell',
    apiUrl: typeof group.apiUrl === 'string' ? group.apiUrl : ''
  };
}
...
class SmartShellDisplay {
  ...
  normalizeSettings(settings) {
    ...
    const panelOpacity = Number(settings.panelOpacity);
    ...
    return {
      ...
      logoImage: typeof settings.logoImage === 'string' && settings.logoImage.trim()
        ? settings.logoImage
        : DEFAULT_LOGO_IMAGE,
      palettePreset,
      ...
      panelOpacity: Number.isFinite(panelOpacity) ? Math.min(Math.max(panelOpacity, 0), 95) : 72,
      cardOpacity: Number.isFinite(cardOpacity) ? Math.min(Math.max(cardOpacity, 0), 100) : 58
    };
  }
  ...
  applySettings() {
    ...
    this.elements.panelOpacity.value = this.settings.panelOpacity;
  ...
  applyLogo() {
    this.elements.brandLogo.src = this.settings.logoImage || DEFAULT_LOGO_IMAGE;
  }
  ...
  applyVisualSettings() {
    ...
    root.style.setProperty('--panel-opacity', (this.settings.panelOpacity / 100).toFixed(2));
  ...
  setupEventListeners() {
    ...
    this.elements.logoImage.addEventListener('change', (event) => {
      const [file] = event.target.files || [];
      if (!file) {
        return;
      }
      ...
      const reader = new FileReader();
      reader.onload = (loadEvent) => {
        this.settings.logoImage = loadEvent.target.result;
      };
      reader.readAsDataURL(file);
    });
  ...
  async handleSettingsSave() {
    this.settings = this.normalizeSettings({
      ...
      panelOpacity: Number(this.elements.panelOpacity.value),
  ...
  async fetchData() {
    ...
    try {
      if (!smartshellAPI.hasToken()) {
        const timerHosts = await this.fetchTimerZoneHosts(true);
        this.renderDemoZones('', timerHosts);
      ...
      const smartShellHosts = filteredHosts
        .map((host) => this.transformHost(host, bookingMap.get(host.id) || []))
        .filter(Boolean);

      const timerHosts = await this.fetchTimerZoneHosts(true);

      this.hosts = [...smartShellHosts, ...timerHosts]
        .sort((first, second) => {
          ...
          if (first.number !== second.number) {
            if (first.zoneKey === 'tv' && second.zoneKey === 'tv') {
      ...
      const timerHosts = await this.fetchTimerZoneHosts(true);
      this.renderDemoZones(`Нет данных API. ${error.message}`, timerHosts);
  ...
  getTimerDisplayGroup() {
    return this.getConfiguredDisplayGroups().find((group) => group.source === 'timer') || null;
  }

  async fetchTimerZoneHosts(silent = false) {
    const timerGroup = this.getTimerDisplayGroup();
    if (!timerGroup?.apiUrl) {
      return [];
    }

    try {
      const response = await fetch(timerGroup.apiUrl, { cache: 'no-store' });
      if (!response.ok) {
        throw new Error(`Timer API ${response.status}`);
      }

      const payload = await response.json();
      const hosts = Array.isArray(payload)
        ? payload
      ...
      return hosts
        .map((host, index) => this.transformTimerHost(host, timerGroup, index))
        .filter(Boolean);
    } catch (error) {
      if (!silent) {
        throw error;
      ...
      console.warn('Не удалось получить данные TV-зоны:', error);
      return [];
    }
  }

  transformTimerHost(host, group, index) {
    if (!host) {
      return null;
    }

    const number = Number(host.number);
    const resolvedStatus = host.status === 'busy' ? 'busy' : 'free';

    return {
      id: host.id || `${group.key}-${index + 1}`,
      alias: host.alias || `${group.title} ${index + 1}`,
      number: Number.isFinite(number) ? number : index + 1,
      zoneKey: group.key,
      zoneTitle: group.title,
      zoneOrder: group.order,
      status: resolvedStatus,
      statusLabel: host.statusLabel || (resolvedStatus === 'busy' ? 'Занят до' : 'Свободен'),
  ...
  resolveDisplayGroups(hosts) {
    ...
    const smartShellGroups = configuredGroups.filter((group) => group.source === 'smartshell');
    const externalGroups = configuredGroups.filter((group) => group.source !== 'smartshell');

    if (!smartShellGroups.length) {
      return externalGroups;
    }

    const existingGroupIds = new Set((hosts || []).map((host) => Number(host.group_id)).filter(Number.isFinite));

    const matchedConfiguredGroups = smartShellGroups
      .map((group) => {
    ...
    if (matchedConfiguredGroups.length === smartShellGroups.length) {
      return [...matchedConfiguredGroups, ...externalGroups];
    }
    ...
    const vipTemplate = smartShellGroups.find((group) => group.key === 'vip') || { key: 'vip', title: 'VIP', order: 2 };
    const standardTemplate = smartShellGroups.find((group) => group.key === 'standart-b') || { key: 'standart-b', title: 'Общий зал', order: 1 };
    ...
    const resolvedGroups = smartShellGroups
      .map((group) => resolvedByKey.get(group.key))
      .filter(Boolean);

    return [...(resolvedGroups.length ? resolvedGroups : smartShellGroups), ...externalGroups];
  }
  ...
  filterDisplayHosts(hosts) {
    const allowedIds = new Set(
      this.displayGroups
        .filter((group) => group.source === 'smartshell')
  ...
  getZoneCountLabel(group, count) {
    return group.key === 'tv' ? `${count} консолей` : `${count} хостов`;
  }
  ...
  renderDemoZones(errorMessage = '', timerHosts = []) {
    this.displayGroups = DEFAULT_DISPLAY_GROUPS.map((group, index) => normalizeDisplayGroup(group, index));
    const demoHosts = [
    ...
    const fallbackTimerHosts = [
      { id: 'PS-1', alias: 'PS 1', number: 1, zoneKey: 'tv', zoneTitle: 'TV', zoneOrder: 3, status: 'free', statusLabel: 'Свободен', statusTime: '' },
      { id: 'PS-2', alias: 'PS 2', number: 2, zoneKey: 'tv', zoneTitle: 'TV', zoneOrder: 3, status: 'busy', statusLabel: 'Занят до', statusTime: '21:45' },
      { id: 'PS-3', alias: 'PS 3', number: 3, zoneKey: 'tv', zoneTitle: 'TV', zoneOrder: 3, status: 'free', statusLabel: 'Свободен', statusTime: '' }
    ];

    this.hosts = [...demoHosts, ...(timerHosts.length ? timerHosts : fallbackTimerHosts)];
        ...
        return `
          <section class="zone-section ${group.key}">
            <div class="zone-heading">
              ...
              <p>${this.getZoneCountLabel(group, groupHosts.length)}</p>
            </div>
