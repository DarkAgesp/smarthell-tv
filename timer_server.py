...
DISPLAY_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'smarthell-tv1', 'smarthell-tv', 'smartshell-display')
...
def extract_stick_number(stick_id):
    """Достать номер из названия палки, например PS-3 -> 3"""
    digits = ''.join(char for char in str(stick_id) if char.isdigit())
    return int(digits) if digits else 0


def get_active_timer_for_stick(stick_id, now=None):
    """Вернуть активный таймер для палки, если он есть"""
    current_time = now if now is not None else int(time.time())
    timers = [
        timer for timer in active_timers.values()
        if timer.get('stickId') == stick_id and timer.get('endTime', 0) > current_time
    ]

    if not timers:
        return None

    return min(timers, key=lambda timer: timer.get('endTime', 0))


def build_tv_zone_payload():
    """Собрать данные PS-зоны для TV-экрана"""
    now = int(time.time())
    hosts = []

    for stick_id in sorted(STICKS.keys(), key=extract_stick_number):
        timer = get_active_timer_for_stick(stick_id, now)
        end_time = datetime.fromtimestamp(timer['endTime']).strftime('%H:%M') if timer else ''

        hosts.append({
            'id': stick_id,
            'alias': str(stick_id).replace('-', ' '),
            'number': extract_stick_number(stick_id),
            'status': 'busy' if timer else 'free',
            'statusLabel': 'Занят до' if timer else 'Свободен',
            'statusTime': end_time,
            'timerId': timer.get('id') if timer else None,
            'endTime': timer.get('endTime') if timer else None
        })
...
@app.route('/tv/')
@app.route('/tv/<path:requested_path>')
def tv_display(requested_path='index.html'):
    """Отдать TV-экран и его статические файлы через текущий Flask сервер"""
    if not os.path.isdir(DISPLAY_ROOT):
        return jsonify({'ok': False, 'error': f'Display root not found: {DISPLAY_ROOT}'}), 404

    normalized_path = requested_path.strip('/') or 'index.html'
    target_path = os.path.abspath(os.path.join(DISPLAY_ROOT, normalized_path))

    if not target_path.startswith(os.path.abspath(DISPLAY_ROOT)):
        abort(403)
...
@app.route('/shared/<path:requested_path>')
def shared_asset(requested_path):
    """Отдать общие файлы из корня проекта, например Аватарка.png"""
    if not os.path.isdir(SHARED_ROOT):
        abort(404)

    normalized_path = requested_path.strip('/')
    target_path = os.path.abspath(os.path.join(SHARED_ROOT, normalized_path))

    if not target_path.startswith(SHARED_ROOT):
        abort(403)
...
@app.route('/api/display-zone', methods=['GET'])
def api_display_zone():
    """Данные PS-зоны для внешнего TV-экрана"""
    return jsonify(build_tv_zone_payload())
